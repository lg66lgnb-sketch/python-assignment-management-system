...
\centering
\begin{tikzpicture}
  \node[dataobj] (users) at (0,2.2) {USERS\\用户};
  \node[dataobj] (courses) at (-3.4,0.2) {COURSES\\课程};
  \node[dataobj] (enrollments) at (0,0.2) {ENROLLMENTS\\选课};
  \node[dataobj] (assignments) at (3.4,0.2) {ASSIGNMENTS\\作业};
  \node[dataobj] (submissions) at (3.4,-2.0) {SUBMISSIONS\\提交};
  \draw[line] (users) -- (courses);
  \draw[line] (users) -- (enrollments);
  \draw[line] (courses) -- (enrollments);
  \draw[line] (courses) -- (assignments);
  \draw[line] (assignments) -- (submissions);
  \draw[line] (users) -- ++(0,-3.7) -| (submissions);
  \node[align=left,font=\scriptsize] at (-1.75,-2.35) {关系说明：教师创建课程；学生通过选课加入课程；\\课程发布作业；学生围绕作业形成提交记录。};
\end{tikzpicture}
\caption{数据库实体关系图}
\end{figure}
...
\begin{tikzpicture}[node distance=0.62cm]
  \begin{scope}[xshift=-4.2cm]
    \node[actor] (admin) at (0,2.1) {管理员};
    \node[usecase] (a1) at (0,0.95) {审核教师账号};
    \node[usecase] (a2) at (0,0.0) {停用用户};
    \node[usecase] (a3) at (0,-0.95) {查看基础统计};
    \foreach \x in {a1,a2,a3} \draw[line] (admin) -- (\x);
  \end{scope}
  \begin{scope}[xshift=0cm]
    \node[actor] (teacher) at (0,2.1) {教师};
    \node[usecase] (t1) at (0,0.95) {创建课程};
    \node[usecase] (t2) at (0,0.0) {布置作业};
    \node[usecase] (t3) at (0,-0.95) {查看提交};
    \node[usecase] (t4) at (0,-1.9) {评分导出};
    \foreach \x in {t1,t2,t3,t4} \draw[line] (teacher) -- (\x);
  \end{scope}
  \begin{scope}[xshift=4.2cm]
    \node[actor] (student) at (0,2.1) {学生};
    \node[usecase] (s1) at (0,0.95) {加入课程};
    \node[usecase] (s2) at (0,0.0) {查看作业};
    \node[usecase] (s3) at (0,-0.95) {提交作业};
    \node[usecase] (s4) at (0,-1.9) {查看成绩};
    \foreach \x in {s1,s2,s3,s4} \draw[line] (student) -- (\x);
  \end{scope}
\end{tikzpicture}
\caption{系统用例图}
\end{figure}
...
\begin{tikzpicture}[node distance=1.05cm and 1.1cm]
  \node[flow] (a) {学生登录};
  \node[flow, right=of a] (b) {加入课程};
  \node[flow, right=of b] (c) {查看作业};
  \node[flow, below=of c] (d) {填写说明\\或上传文件};
  \node[decision, left=of d] (e) {已有\\提交?};
  \node[flow, below=of e] (f) {更新提交};
  \node[flow, left=of e] (g) {新增提交};
  \node[flow, right=of d] (h) {教师查看};
  \node[flow, right=of h] (i) {评分评语};
  \draw[arrow] (a) -- (b);
  \draw[arrow] (b) -- (c);
  \draw[arrow] (c) -- (d);
  \draw[arrow] (d) -- (e);
  \draw[arrow] (e) -- node[left]{是} (f);
  \draw[arrow] (e) -- node[above]{否} (g);
  \draw[arrow] (f) -| (h);
  \draw[arrow] (g) |- (h);
  \draw[arrow] (h) -- (i);
\end{tikzpicture}
